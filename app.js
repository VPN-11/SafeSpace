...
const expressionFaceMood = document.getElementById("expressionFaceMood");
const expressionFacePrimary = document.getElementById("expressionFacePrimary");
const expressionFaceStrength = document.getElementById("expressionFaceStrength");
const expressionFaceNote = document.getElementById("expressionFaceNote");
...
function normalizeExpressionScores(expressions = {}) {
  ...
  const rawEntries = Object.entries(expressions || {}).reduce((acc, [key, value]) => {
    ...
    acc[keyMap[normalizedKey] || normalizedKey] = value;
    return acc;
  }, {});

  const values = keys.map((key) => parseValue(rawEntries[key]));
  ...
  return keys.reduce((acc, key, index) => ({
    ...acc,
    [key]: Number((values[index] / total).toFixed(4)),
  }), {});
}
...
function safeNormalizeExpressionScores(expressions = {}) {
  ...
  const values = keys.map((key) => parseValue(expressions?.[key] ?? expressions?.[key.toLowerCase()]));
  const total = values.reduce((sum, value) => sum + value, 0);
  if (total <= 0) {
    return null;
  }
...
// Robust fallback parser: try multiple heuristics to extract a normalized
// expression score map from different input shapes (0-1 floats, 0-100
// percentages, or raw counts). Returns null only when no usable values.
function robustNormalizeExpressions(expressions = {}) {
  const keys = ["neutral", "happy", "sad", "angry", "fearful", "disgusted", "surprised"];

  // 1) Prefer existing normalized/safe path
  const safe = safeNormalizeExpressionScores(expressions) || normalizeExpressionScores(expressions);
  if (safe) return safe;

  // 2) Try simple numeric extraction and normalize by total
  const raw = keys.map((k) => {
    const v = expressions?.[k] ?? expressions?.[k.toLowerCase()];
    const n = Number(v);
    return Number.isFinite(n) ? Math.max(n, 0) : 0;
  });
  const sum = raw.reduce((s, x) => s + x, 0);
  if (sum > 0) {
    return keys.reduce((acc, key, i) => ({ ...acc, [key]: Number((raw[i] / sum).toFixed(4)) }), {});
  }
...
function enhanceExpressionScoresWithBaseline(expressions = {}) {
  const keys = ["neutral", "happy", "sad", "angry", "fearful", "disgusted", "surprised"];
  const normalized = safeNormalizeExpressionScores(expressions) ||
    keys.reduce((acc, key) => ({ ...acc, [key]: Number(expressions?.[key] || 0) }), {});

  const sharpenFactor = 1.15;
  const boosted = keys.map((key) => Math.pow(normalized[key], sharpenFactor));
  ...
  return keys.reduce((acc, key, index) => ({
    ...acc,
    [key]: Number((boosted[index] / total).toFixed(4)),
  }), {});
}

function buildExpressionConfidenceMetrics(expressions = {}) {
  const keys = ["neutral", "happy", "sad", "angry", "fearful", "disgusted", "surprised"];
  const normalized = normalizeExpressionScores(expressions) || keys.reduce((acc, key) => ({ ...acc, [key]: 0 }), {});
  ...
  return { top: topPercent, margin: marginPercent, reliability };
}

function buildFaceMoodSnapshot(expressions = {}) {
  ...
  const positiveSignal = happy + neutral * 0.4 + surprised * 0.2;
  const challengingSignal = sad + angry + fearful + disgusted * 0.7;
  const topExpression = Object.entries(expressions)
    .sort((a, b) => b[1] - a[1])[0] || ["neutral", 0];
  const primaryLabel = titleCase(topExpression[0]);
  const primaryPercent = Math.round(topExpression[1] * 100);
  const moodLabel = positiveSignal >= challengingSignal ? (positiveSignal >= 0.5 ? "Positive face mood" : "Balanced face mood") : "Challenging face mood";
  const signalStrength = Math.round(Math.max(positiveSignal, challengingSignal, topExpression[1]) * 100);
  ...
  return {
    ...
    primaryPercent,
    signalStrength,
    positivePercent: Math.round(positiveSignal * 100),
    challengingPercent: Math.round(challengingSignal * 100),
  };
}

function renderExpressionConfidence(expressions = null, source = "none", confidence = null) {
  const keys = ["neutral", "happy", "sad", "angry", "fearful", "disgusted", "surprised"];
  const norm = robustNormalizeExpressions(expressions) || keys.reduce((acc, key) => ({ ...acc, [key]: 0 }), {});
  const adjustedExpressions = enhanceExpressionScoresWithBaseline(norm) || keys.reduce((acc, key) => ({ ...acc, [key]: 0 }), {});
  const normalized = keys.map((key) => ({ key, value: adjustedExpressions[key] || 0 }));
  const snapshot = buildFaceMoodSnapshot(adjustedExpressions);
  ...
  expressionFaceMood.textContent = `${snapshot.moodLabel} (${snapshot.positivePercent}% positive)`;
  expressionFacePrimary.textContent = `${snapshot.primaryLabel} (${snapshot.primaryPercent}%)`;
  expressionFaceStrength.textContent = `${snapshot.signalStrength}% overall signal`;
  expressionFaceNote.textContent =
    source === "none"
      ? "Start camera and capture your facial expression to see how your mood signal is reflected in your current frame."
      : "This snapshot uses face expression scores to map your current emotion into a clear mood summary for support.";
}
  ...
  if (faceApiReady && window.faceapi) {
    try {
      ...
      if (detection?.expressions) {
        const normalizedExpressions = robustNormalizeExpressions(detection.expressions) || {};
        const enhancedExpressions = enhanceExpressionScoresWithBaseline(normalizedExpressions);
        latestExpressionScores = enhancedExpressions;
        renderExpressionConfidence(enhancedExpressions, "live");
        const sorted = Object.entries(enhancedExpressions || {}).sort((a, b) => b[1] - a[1]);
